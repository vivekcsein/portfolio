...
const CodePage = () => {
  const totalDocuments = codeConfig.categories.reduce(
    ...
    <main className="content-page">
      ...
      <div className="content-container">
        ...
        <header className="content-hero">
          ...
          <h1 className="content-title">{codeConfig.title}</h1>

          <p className="content-description">{codeConfig.description}</p>

          <div className="content-meta">
            <span>
              {codeConfig.categories.length}{" "}
              {codeConfig.categories.length === 1 ? "category" : "categories"}
            </span>
        ...
        <section className="content-section" aria-labelledby="content-heading">
          <div className="content-section-header">
            <div>
              ...
              <h2 id="content-heading">Browse my Code and Resources</h2>
            </div>

            <span className="content-count">
              {String(codeConfig.categories.length).padStart(2, "0")}
            </span>
          </div>

          <ol className="content-grid" aria-label="Documentation categories">
            {codeConfig.categories.map((category, index) => (
              <Link
                key={category.key}
                href={`/${codeConfig.path}/${category.key}`}
